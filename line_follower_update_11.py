...
class TurtleBotBrain(Node):
    def __init__(self):
        ...
        self.obstacle_approach_offset = 0.18 # 10cm gap to leave between robot and obstacle
        ...
        self.obstacle_square_angle_tolerance = 15.0  # degrees - since the board is parallel to the line, keep creeping forward until the lidar bearing is within this of 90 deg (robot directly abeam the board) before locking in the reading and turning. Locking in too early (board still ahead-and-to-the-side) overstates the true perpendicular distance.
    ...
    def fsm_loop(self):
        ...
        if self.current_state == RobotState.FOLLOW_LINE:
            ...
            angle_is_abeam = (self.obstacle_info_received and
                               abs(abs(self.obstacle_target_angle_deg) - 90.0) <= self.obstacle_square_angle_tolerance)
            ...
            if distance < self.target_reverse_distance:
                ...
                if self.obstacle_needs_full_retrace:
                    self.get_logger().info(
