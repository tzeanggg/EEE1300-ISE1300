...
class TurtleBotBrain(Node):
    def __init__(self):
        ...
        self.obstacle_angular_speed = 0.8     # rad/s, matches EXECUTE_TURN/ESCAPE_TURN rate

        # --- Topple / Return-to-Front Settings ---
        self.topple_wait_duration = 2.0   # seconds to let the actuator topple the obstacle
    ...
    def fsm_loop(self):
        ...
        if self.current_state == RobotState.FOLLOW_LINE:
            ...
            # stays in place), then rotate back to face front, BEFORE the
            # existing reverse-out logic below runs. ---
            if not self.returned_to_front:
                if self.topple_wait_start == 0.0:
                    self.topple_wait_start = time.time()
                    self.get_logger().info(
                        f'Toppling obstacle. Waiting {self.topple_wait_duration}s for actuator...')

                elapsed_topple = time.time() - self.topple_wait_start
                if elapsed_topple < self.topple_wait_duration:
                    # Hold still while the actuator does its thing.
                    twist.linear.x = 0.0
                    twist.angular.z = 0.0
                else:
                    # Rotate back the same amount, opposite direction, to
                    # undo the SEARCH_OBSTACLE rotation and re-face front.
                    ...
                    if (time.time() - self.return_rotate_start_time) >= self.obstacle_rotate_duration:
                        self.get_logger().info('Facing front again. Beginning reverse-out.')
